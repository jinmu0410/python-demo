...
def saveExcel(data, file_name, sheet_name):
    ...
    if os.path.isfile(file_name):
        try:
            existing_data = pd.read_excel(file_path, sheet_name)
            start_row = existing_data[sheet_name].shape[0] + 1
            print("读取文件" + sheet_name + "-rowNum-" + start_row)
    ...
    print("保存成功" + file_name + " ---sheet_name" + sheet_name)
...
if __name__ == '__main__':
    # 读取本地JSON文件 例如云南.json
    file_path = sys.args[1]  # 替换成你的JSON文件路径
